...
| `/api/payment-orders` | GET·POST | 회원 | 본인 주문 조회·상품 신청. 병원 광고는 로그인한 병원 회원만 신청할 수 있고, 결제 대기 공고(`draft`)도 같은 D1 배치로 생성 |
...
- **역할 게이트**: 광고와 열람권은 로그인한 병원 회원만 구매합니다(비로그인 401, 의료인·관리자 403). 사업자등록증 제출 이력은 구매 권한을 승인·반려하는 조건으로 사용하지 않습니다.
