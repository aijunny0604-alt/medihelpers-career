...
function Footer({ operations }) {
  return <footer>
    <div className="footer-grid">
      ...
      <div className="footer-column"><strong>안내</strong><Link to="/medical-staff">의료인 채용</Link><Link to="/signup">로그인·회원가입</Link><Link to="/terms">이용약관</Link><Link to="/privacy">개인정보처리방침</Link><Link to="/refund">환불 정책</Link><Link to="/withdrawal">회원 탈퇴 약관</Link></div>
    </div>
...
function ConsultationForm({ initialRole = 'doctor', initialContext = '', initialProfession = '', initialTopic = '' }) {
  ...
  return <form className="consult-form guided" onSubmit={submit}>
    ...
    {step === 1 && <div className="consult-step"><div className="role-tabs-label"><small>먼저, 어느 쪽으로 상담받으시나요?</small><p>{role === 'doctor' ? '이직·구직을 원하는 의사·의료인이라면 「의사」를 선택하세요.' : '의료진을 채용하려는 병원·기관이라면 「병원」을 선택하세요.'}</p></div><div className="role-tabs"><button type="button" className={role === 'doctor' ? 'active' : ''} onClick={() => changeRole('doctor')}><Stethoscope /> 의사·의료인 (구직)</button><button type="button" className={role === 'hospital' ? 'active' : ''} onClick={() => changeRole('hospital')}><Building2 /> 병원·기관 (채용)</button></div><div className="step-question"><small>하나만 선택해주세요</small><h4>어떤 상담이 필요하신가요?</h4></div><div className="choice-grid">{topics.map((topic) => <button type="button" key={topic} className={data.topic === topic ? 'selected' : ''} onClick={() => update('topic', topic)}><span>{topic}</span>{data.topic === topic && <Check />}</button>)}</div><button type="button" className="button primary full" disabled={!data.topic} onClick={() => setStep(2)}>다음 · 필요한 조건 입력 <ArrowRight /></button></div>}
          ...
          {visibleTalent.map((person) => (
            <article className="talent-card" key={person.code}>
              <div className="talent-top">
                ...
                <div>
                  {person.verifiedByHeadhunter && <span className="tag tag-verified talent-verified-row"><ShieldCheck /> 헤드헌터 인증</span>}
