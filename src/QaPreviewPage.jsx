...
export default function QaPreviewPage({ qa }) {
  ...
  return <>
    <section className="page-hero qa-hero">
      <div className="page-hero-inner"><span className="eyebrow"><ShieldCheck /> ROLE QA PREVIEW</span><h1>권한별 화면을<br />직접 확인하세요</h1><p>실제 로그인이나 결제 없이 관리자·병원·의료인·멤버십 상태의 화면 차이를 안전하게 점검합니다.</p></div>
    </section>
    <section className="section qa-preview-page">
      ...
      <div className={`qa-dashboard tone-${current.tone}`}>
        <div className="qa-dashboard-copy"><small>CURRENT TEST SESSION</small><h2>{current.label} 화면</h2><p>{current.description}</p><div className="qa-capabilities">{Object.entries(current.capabilities).filter(([, enabled]) => enabled).map(([key]) => <span key={key}><Check /> {({ signedIn:'로그인',admin:'관리자',hospital:'병원 권한',doctor:'의료인 권한',membership:'멤버십 활성',privateDetails:'상세조건 열람' })[key]}</span>)}</div></div>
