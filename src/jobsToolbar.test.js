...
test('병원 채용 페이지 중간 도구 모음에는 공고 등록만 남긴다', async () => {
  ...
  assert.doesNotMatch(jobs, /DOCTOR RECRUITMENT|조건부터 비교하는|jobs-hero/);
});

test('채용정보 제목과 공고 등록 버튼은 가까이 정렬된 큰 주 행동으로 표시한다', async () => {
  const styles = await readFile(new URL('./styles.css', import.meta.url), 'utf8');

  assert.match(styles, /\.jobs-hub-nav\{margin-top:76px\}/);
  assert.match(styles, /@media\(max-width:780px\)[\s\S]*?\.jobs-hub-nav\{margin-top:66px\}/);
