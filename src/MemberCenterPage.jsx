...
export default function MemberCenterPage({ route, qa, auth }) {
  ...
  const [serverData, setServerData] = useState({ consultations: [], alerts: [], unreadCount: 0, activity: [], orders: [], resume: null, jobSeekerPosts: [], recommendedCandidates: [], talentCredits: { total:0, used:0, remaining:0 } });
  ...
  useEffect(() => {
    ...
    if (auth?.status === 'guest') {
      ...
      setProfile(null);
      setServerData({ consultations: [], alerts: [], unreadCount: 0, activity: [], orders: [], resume: null, jobSeekerPosts: [], recommendedCandidates: [], talentCredits: { total:0, used:0, remaining:0 } });
      return;
    }
  ...
  const metrics = qa.active ? demo.metrics : role === 'hospital'
    ? [['진행 중 공고', `${activeAds}건`, '노출 중 공고'], ['새 문의', `${inquiries.filter((item) => item.status === '답변 대기').length}건`, '확인 필요'], ['잔여 열람권', `${Number(serverData.talentCredits?.remaining || 0)}건`, '구매 수량만 차감'], ['추천 후보', `${recommendedCount}명`, '동의 후 연결']]
