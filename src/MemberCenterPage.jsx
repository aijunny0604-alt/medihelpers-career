...
const hospitalDemo = {
  ...
  ads: [
    { title: '정형외과 전문의 추천 초빙', plan: '메인 광고', status: '게시 중', period: '2026.07.17 ~ 08.15', views: 284, inquiries: 7 },
  ...
  payments: [
    { id: 'PAY-260717-01', item: '메인 광고 30일', amount: '149,000원', date: '2026.07.17', status: '결제 완료' },
  ...
  activity: [
    ...
    ['07.17', '메인 광고가 게시되었습니다.', '30일 노출 시작'],
...
function statusClass(value = '') {
  if (/완료|게시 중|노출 중|이용 중|연결/.test(value)) return 'good';
  if (/대기|확인|조율/.test(value)) return 'wait';
  if (/종료|비공개/.test(value)) return 'ended';
  return '';
}
...
function HeadhunterMessagePanel({ role, profile, professionType = '', specialty = '', qa = false }) {
  ...
  if (!allowed) return <section className="member-panel headhunter-message-panel is-restricted">
    <div className="headhunter-message-title"><span><LockKeyhole /></span><div><small>DOCTOR HEADHUNTING</small><h3>의사 회원만 구직 상담을 이용할 수 있습니다</h3><p>헤드헌터 구직 상담은 회원가입 직군이 ‘의사’로 등록된 의료인에게만 제공됩니다.</p></div></div>
  </section>;

  const submit = async (event) => {
    event.preventDefault();
    setError('');
    if (role === 'doctor' && !resumeId) { setError('헤드헌터에게 전달할 이력서를 선택해 주세요.'); return; }
    if (!fieldSpecialty.trim() || !phone.trim() || !message.trim()) { setError('전문과목·연락처·문의 내용을 모두 입력해 주세요.'); return; }
    setBusy(true);
    try {
      if (qa) {
        setSubmittedId('QA-CONSULTATION');
        return;
      }
      const payload = {
        submissionChannel:'mypage_headhunter',
        subject:subject.trim(),
        specialty:fieldSpecialty.trim(),
        phone:phone.trim(),
        message:message.trim(),
        contactTime:'마이페이지 메시지',
        ...(role === 'hospital'
          ? { hospital:profile.organization || profile.displayName }
          : { name:profile.displayName, resumeId })
      };
      const response = await fetch(withBase('/api/consultations'), { method:'POST', credentials:'same-origin', headers:{ 'content-type':'application/json' }, body:JSON.stringify({ requestType:role, payload, privacyConsent:new FormData(event.currentTarget).get('privacy') === 'agreed', privacyVersion:PRIVACY_FORM_VERSION }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || '메시지를 보내지 못했습니다.');
      setSubmittedId(result.id);
      window.dispatchEvent(new CustomEvent('medihelpers:notifications-changed'));
    } catch (reason) {
      setError(reason.message || '메시지를 보내지 못했습니다.');
    } finally {
      setBusy(false);
    }
  };

  if (submittedId) return <section className="member-panel headhunter-message-panel is-success">
    <CircleCheck /><div><small>MESSAGE SENT</small><h3>헤드헌터에게 문의를 보냈습니다</h3><p>{role === 'doctor' ? '선택한 이력서와 구직 문의가 접수 당시 내용으로 함께 보관됩니다.' : '병원 채용 문의가 관리자 상담함에 접수되었습니다.'}</p><a className="button outline" href={withBase(`/mypage/inquiries/${encodeURIComponent(submittedId)}`)}>접수 내용 보기 <ArrowRight /></a></div>
  </section>;

  return <section className="member-panel headhunter-message-panel">
    <div className="headhunter-message-title"><span><MessageCircle /></span><div><small>1:1 HEADHUNTER MESSAGE</small><h3>헤드헌터에게 메시지 보내기</h3><p>{role === 'hospital' ? '필요한 의사와 채용 조건을 관리자에게 바로 문의하세요.' : '저장한 이력서를 선택하고 구직 희망 조건을 관리자에게 안전하게 전달하세요.'}</p></div></div>
    <form onSubmit={submit}>
      <div className="headhunter-message-fields"><label><span>문의 제목 <em>필수</em></span><input value={subject} onChange={(event) => setSubject(event.target.value)} maxLength="120" /></label><label><span>{role === 'hospital' ? '채용 진료과' : '전문과목'} <em>필수</em></span><input value={fieldSpecialty} onChange={(event) => setFieldSpecialty(event.target.value)} placeholder="예: 소화기내과" maxLength="200" /></label><label><span>연락처 <em>필수</em></span><input value={phone} onChange={(event) => setPhone(event.target.value)} type="tel" placeholder="010-0000-0000" maxLength="40" /></label></div>
      <label className="headhunter-message-body"><span>문의 내용 <em>필수</em></span><textarea value={message} onChange={(event) => setMessage(event.target.value)} rows="5" maxLength="3000" placeholder={role === 'hospital' ? '채용하려는 진료과, 시기와 꼭 필요한 조건을 적어주세요.' : '희망 지역, 근무형태, 입사 가능 시기와 상담받고 싶은 내용을 적어주세요.'} /></label>
      {role === 'doctor' && <ResumeSubmitPicker selectedId={resumeId} onSelect={setResumeId} optional={false} />}
      <PrivacyNotice scope="consultation" /><label className="consent"><input required type="checkbox" name="privacy" value="agreed" /><span>[필수] 위 상담 개인정보 수집·이용에 동의합니다.</span></label>
      {error && <p className="headhunter-message-error" role="alert"><TriangleAlert /> {error}</p>}
      <div className="headhunter-message-submit"><p><ShieldCheck /> 문의와 이력서는 메디헬퍼스 헤드헌터 상담 목적으로만 전달됩니다.</p><button className="button primary" type="submit" disabled={busy}>{busy ? '전송 중…' : role === 'hospital' ? '채용 문의 보내기' : '이력서와 구직 문의 보내기'} <ArrowRight /></button></div>
    </form>
  </section>;
}
...
export default function MemberCenterPage({ route, qa, auth }) {
  ...
  const requestedInquiryId = routePath.startsWith('/mypage/inquiries/')
    ...
    : params.get('inquiry') || '';
  const requestedAdEditId = routePath.startsWith('/mypage/ads/') && routePath.endsWith('/edit')
    ? decodeURIComponent(routePath.slice('/mypage/ads/'.length, -'/edit'.length))
    : '';
  ...
  const [serverData, setServerData] = useState({ consultations: [], alerts: [], unreadCount: 0, activity: [], orders: [], resume: null, jobSeekerPosts: [], recommendedCandidates: [], unlockedTalents: [], talentCredits: { total:0, used:0, remaining:0 } });
  ...
  useEffect(() => {
    ...
    if (auth?.status === 'guest') {
      ...
      setProfile(null);
      setServerData({ consultations: [], alerts: [], unreadCount: 0, activity: [], orders: [], resume: null, jobSeekerPosts: [], recommendedCandidates: [], unlockedTalents: [], talentCredits: { total:0, used:0, remaining:0 } });
      return;
    }
  ...
  const currentProfile = {
    ...
    displayName: profile?.displayName || profile?.name || auth?.profile?.name || accountState.identity.displayName || profileBase.displayName || ''
  };
  ...
  const inquiries = qa.active ? demo.inquiries : serverData.consultations.map((item) => {
    ...
    return {
      ...
      subject: directApplication ? `${payload.jobTitle || item.specialty || '채용공고'} 지원` : headhuntInquiry ? `${payload.headhuntPostTitle || item.specialty || '맞춤 공고'} 헤드헌터 문의` : payload.subject || `${item.specialty || payload.professionalType || '채용'} 상담 신청`,
      source: item.id,
  ...
  const ads = qa.active ? demo.ads : serverData.orders
    ...
    .filter((item,index,items) => !item.contentRecordId || items.findIndex(candidate=>candidate.contentRecordId===item.contentRecordId)===index)
    .map((item) => {
      const adStatus = item.adStatus || '';
      const terminalStatus = ({refunded:'환불',cancelled:'취소',failed:'결제 실패',pending_review:'결제 대기',awaiting_payment:'결제 대기'})[item.status];
      const status = terminalStatus || (adStatus === 'published' && item.status === 'paid'
      ...
      return {
        ...
        contentRecordId: item.contentRecordId || '',
        title: item.adTitle || item.productName,
        subtitle: item.adSubtitle || '',
        contentType: item.adContentType || '',
        payload: item.adPayload || {},
        updatedAt: item.adUpdatedAt || '',
  ...
  useEffect(() => {
    if (role !== 'doctor' || qa.active) return;
    let active=true;
    setSavedResumes([]);
    fetch(withBase('/api/resumes'),{credentials:'same-origin'}).then(response => response.ok ? response.json() : null).then(result=>{if(active && result)setSavedResumes(result.resumes || []);}).catch(()=>{});
    return ()=>{active=false;};
  },[role,qa.active,auth?.email]);
  const resumeCards = qa.active ? demo.ads : (savedResumes.length ? savedResumes : serverData.resume ? [serverData.resume] : []).map(resume => ({
    id:resume.id,title:resume.title,plan:`완성도 ${Number(resume.completion)||0}% · 개인 보관`,status:'비공개',period:`최근 수정 ${String(resume.updatedAt||'').slice(0,10)}`,views:'-',inquiries:'-'
  }));
  ...
  const deleteJobSeekerPost = async (post) => {
    if (!post?.id || !await confirmAction('이 구직글을 삭제할까요? 연결된 이력서는 삭제되지 않습니다.')) return;
    try {
  ...
  const unlockedTalents = role === 'hospital' ? (serverData.unlockedTalents || []) : [];
  ...
  const metrics = qa.active ? demo.metrics : role === 'hospital'
    ? [['진행 중 공고', `${activeAds}건`, '노출 중 공고'], ['새 문의', `${inquiries.filter((item) => item.status === '답변 대기').length}건`, '확인 필요'], ['잔여 열람권', `${Number(serverData.talentCredits?.remaining || 0)}건`, `열람 완료 ${unlockedTalents.length}명`], ['추천 후보', `${recommendedCount}명`, '동의 후 연결']]
  ...
  const selectedOwnedAd = requestedAdEditId ? ads.find((item) => item.contentRecordId === requestedAdEditId) : null;
  ...
  const selectMemberTab = (id) => {
    if (requestedInquiryId || requestedAdEditId) {
  ...
  const openMetricDetail = (label) => {
    const target = /진행 중 공고/.test(label) ? { tab:'ads' }
      : /누적 결제/.test(label) ? { tab:'payments' }
        : /잔여 열람권|열람 완료/.test(label) ? { tab:'payments', anchor:'member-unlocked-talents' }
    ...
    if (target.anchor) window.setTimeout(() => document.getElementById(target.anchor)?.scrollIntoView({ behavior:'auto', block:'start' }), 60);
    else window.setTimeout(() => document.querySelector('.member-workspace')?.scrollIntoView({ behavior:'auto', block:'start' }), 30);
  };
  ...
  return <div className={`member-center role-${role}`}>
    ...
    <header className="member-center-hero"><div><small>MY MEDIHELPERS</small><h1>{role === 'hospital' ? currentProfile.organization || currentProfile.displayName : currentProfile.displayName}님, 반갑습니다</h1><p>{role === 'hospital' ? '공고 반응부터 후보 문의와 결제 내역까지 병원 채용 업무를 한곳에서 관리하세요.' : '이력서와 상담 제안, 관심 공고와 활동 내역을 한곳에서 관리하세요.'}</p></div><div className="member-identity"><span>{role === 'hospital' ? <Building2 /> : <Stethoscope />}</span><div><small>현재 로그인</small><strong>{roleLabel}</strong><em><BadgeCheck /> 본인 확인</em></div></div></header>
    <div className="member-center-shell">
      <aside className="member-side"><div className="member-mini-profile"><span>{role === 'hospital' ? <Building2 /> : <UserRound />}</span><div><strong>{role === 'hospital' ? currentProfile.organization : `${currentProfile.displayName || '회원'} 님`}</strong><small>{currentProfile.jobTitle}</small></div></div><nav>{nav.map(([id, label, Icon]) => <button key={id} type="button" className={(requestedInquiryId ? id === 'inquiries' : requestedAdEditId ? id === 'ads' : tab === id) ? 'active' : ''} onClick={() => selectMemberTab(id)}><Icon />{label}{id === 'notifications' && serverData.unreadCount > 0 && <b className="member-nav-badge">{serverData.unreadCount > 99 ? '99+' : serverData.unreadCount}</b>}<ChevronRight /></button>)}</nav><button type="button" className="member-side-logout" onClick={signOut} disabled={signingOut}><LogOut /> {signingOut ? '로그아웃 중…' : '로그아웃'}</button></aside>
      <main className="member-workspace">
        {requestedAdEditId ? (selectedOwnedAd
          ? <HospitalAdEditPage ad={selectedOwnedAd} />
          : <section className="member-empty member-empty-large"><BriefcaseBusiness /><strong>수정할 공고를 찾을 수 없습니다</strong><p>삭제되었거나 현재 병원 계정에서 소유한 공고가 아닙니다.</p><a className="button primary" href={withBase('/mypage?tab=ads')}>내 공고 목록으로 돌아가기</a></section>) : requestedInquiryId ? (selectedInquiry
        ...
        {tab === 'overview' && <>
          <div className="member-page-head"><div><small>{role === 'hospital' ? 'HOSPITAL DASHBOARD' : 'DOCTOR DASHBOARD'}</small><h2>내 활동 요약</h2><p>확인이 필요한 항목을 먼저 모았습니다.</p></div><a className="button primary" href={withBase(role === 'hospital' ? '/advertise' : serverData.resume ? '/resume' : `/resume?new=1&next=${encodeURIComponent('/mypage?tab=resume')}`)}>{role === 'hospital' ? '새 공고 등록' : serverData.resume ? '이력서 관리' : '이력서 만들기'} <ArrowRight /></a></div>
        ...
        {tab === 'inquiries' && <><div className="member-page-head"><div><small>MESSAGES & MATCHING</small><h2>{role === 'hospital' ? '문의·후보 연결' : '상담·제안 내역'}</h2><p>{role === 'hospital' ? '채용 지원과 접수한 문의 내역을 확인합니다.' : '지원한 공고와 접수한 문의의 진행 상태를 확인합니다.'}</p></div></div>{inquiries.length ? <section className="member-panel member-table-panel"><div className="member-table-head"><span>보낸 사람</span><span>문의 내용</span><span>접수일</span><span>상태</span></div>{inquiries.map((item) => <button type="button" className="member-inquiry-row" key={`${item.source}-${item.subject}`} onClick={() => openInquiry(item)}><strong>{item.name}</strong><div><b>{item.subject}</b><small>{memberFacingInquiryLabel(item)}</small></div><time>{item.time}</time><span className="member-inquiry-state"><em className={statusClass(item.status)}>{item.status}</em><ChevronRight /></span></button>)}</section> : <div className="member-empty member-empty-large"><MessageCircle /><strong>아직 상담·문의 내역이 없습니다</strong><p>새로운 상담이나 문의가 접수되면 진행 상태와 함께 표시됩니다.</p></div>}<div className="member-privacy-note"><ShieldCheck /><div><strong>{role === 'hospital' ? '의사 실명과 연락처는 동의 후 공개됩니다' : '내 실명과 연락처는 동의한 병원에만 전달됩니다'}</strong><p>메디헬퍼스 헤드헌터가 연결 범위를 확인한 뒤 필요한 정보만 안전하게 전달합니다.</p></div></div></>}
