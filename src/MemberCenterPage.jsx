...
import {
  ArrowRight, BadgeCheck, Bell, BriefcaseBusiness, Building2, CalendarDays,
  Check, ChevronRight, CircleCheck, CreditCard, FileText, Heart, KeyRound,
  LockKeyhole, Mail, MessageCircle, Receipt, Settings, ShieldCheck, Stethoscope,
  UserRound
} from 'lucide-react';
...
const hospitalDemo = {
  ...
  inquiries: [
    { name: '박○○ 의사', subject: '근무시간·토요일 격주 여부 문의', source: '정형외과 집중 초빙', time: '오늘 10:24', status: '답변 대기' },
    { name: '메디헬퍼스 헤드헌터', subject: '후보 2명 조건 확인 요청', source: '비공개 추천', time: '어제 16:40', status: '확인 필요' },
    { name: '이○○ 의사', subject: '급여와 입사 가능 시점 상담 신청', source: '정형외과 집중 초빙', time: '07.15', status: '상담 연결' }
  ],
  payments: [
...
const doctorDemo = {
  ...
  inquiries: [
    { name: '메디헬퍼스 헤드헌터', subject: '부산권 정형외과 조건 확인', source: '비공개 이직 상담', time: '오늘 09:10', status: '답변 대기' },
    { name: '해운대바른척추병원', subject: '면접 가능 일정 요청', source: '동의 후 병원 연결', time: '어제 14:20', status: '일정 조율' }
  ],
  payments: [
...
export default function MemberCenterPage({ route, qa }) {
  ...
  const [serverData, setServerData] = useState({ consultations: [], activity: [] });
  ...
  useEffect(() => {
    ...
    fetch('/api/member-center', { credentials: 'same-origin', headers: { accept: 'application/json' } })
      ...
      .then((data) => {
        ...
        setServerData({ consultations: data.consultations || [], activity: data.activity || [] });
      })
  ...
  const inquiries = qa.active ? demo.inquiries : serverData.consultations.map((item) => ({ name: '메디헬퍼스 상담', subject: `${item.specialty || '채용'} 상담 신청`, source: item.id, time: String(item.createdAt || '').slice(0, 10), status: ({ new:'답변 대기', contacted:'연락 완료', in_progress:'상담 연결', closed:'종료' })[item.status] || item.status }));
  ...
  const payments = qa.active ? demo.payments : [];
    ...
    <div className="member-center-shell">
      ...
      <main className="member-workspace">
        {tab === 'overview' && <>
          ...
          <section className="member-panel"><div className="member-panel-head"><div><h3>지금 확인할 항목</h3><p>새로운 문의와 진행 상태를 놓치지 마세요.</p></div><button onClick={() => setTab('inquiries')}>전체 보기 <ArrowRight /></button></div>{inquiries.length ? <div className="member-inquiry-list">{inquiries.slice(0, 3).map((item) => <article key={item.subject}><span><MessageCircle /></span><div><small>{item.name} · {item.time}</small><strong>{item.subject}</strong><p>{item.source}</p></div><em className={statusClass(item.status)}>{item.status}</em></article>)}</div> : <div className="member-empty"><MessageCircle /><strong>아직 접수된 문의가 없습니다</strong><p>새 문의가 들어오면 이곳에 바로 표시됩니다.</p></div>}</section>
        ...
        {tab === 'inquiries' && <><div className="member-page-head"><div><small>MESSAGES & MATCHING</small><h2>{role === 'hospital' ? '문의·후보 연결' : '상담·제안 내역'}</h2><p>{role === 'hospital' ? '의사 문의와 헤드헌터의 후보 추천을 확인합니다.' : '헤드헌터 상담과 병원 제안 진행 상태를 확인합니다.'}</p></div></div>{inquiries.length ? <section className="member-panel member-table-panel"><div className="member-table-head"><span>보낸 사람</span><span>문의 내용</span><span>접수일</span><span>상태</span></div>{inquiries.map((item) => <article key={`${item.source}-${item.subject}`}><strong>{item.name}</strong><div><b>{item.subject}</b><small>{item.source}</small></div><time>{item.time}</time><em className={statusClass(item.status)}>{item.status}</em></article>)}</section> : <div className="member-empty member-empty-large"><MessageCircle /><strong>아직 상담·문의 내역이 없습니다</strong><p>새로운 상담이나 문의가 접수되면 진행 상태와 함께 표시됩니다.</p></div>}<div className="member-privacy-note"><ShieldCheck /><div><strong>{role === 'hospital' ? '의사 실명과 연락처는 동의 후 공개됩니다' : '내 실명과 연락처는 동의한 병원에만 전달됩니다'}</strong><p>메디헬퍼스 헤드헌터가 연결 범위를 확인한 뒤 필요한 정보만 안전하게 전달합니다.</p></div></div></>}
