...
test('inquiry details use a standalone member-center page and sanitize corrupted legacy text', async () => {
  ...
  assert.match(memberCenter, /const isDirectApplication = Boolean\(inquiry\.directApplication\)/);
  assert.doesNotMatch(memberCenter, /inquiry\.messages/);
  assert.doesNotMatch(memberCenter, /inquiry-detail-reply/);
