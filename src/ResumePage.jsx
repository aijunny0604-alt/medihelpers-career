...
export default function ResumePage() {
  ...
  const chooseCertificateFiles = (files) => {
    ...
    setCertificateError(
      valid.length === selected.length
        ? ''
        : 'PDF·JPG·PNG 파일만 장당 10MB 이하, 최대 5개까지 등록할 수 있습니다.'
    );
    setCertificateFiles(valid);
  };
      ...
      <section className="resume-editor">
        {activeStep === 'basic' && <div className="resume-step-panel">
          ...
          <div className="resume-basic-profile">
            <div className={`resume-photo-field ${dragTarget === 'photo' ? 'is-dragging' : ''}`} {...dropZoneProps('photo', (files) => chooseProfilePhoto(files?.[0]))}>
              <span className="resume-photo-label">프로필 사진 <i>선택사항</i></span>
              ...
              <small>선택사항 · 예: 600×800px · 3:4 세로형 · JPG·PNG·WEBP · 최대 5MB</small>
        ...
        {activeStep === 'intro' && <div className="resume-step-panel"><div className="resume-panel-head"><small>STEP 05</small><h2>소개와 공개설정</h2><p>자체 이력서로 지원하거나 기존 이력서 파일을 함께 첨부할 수 있습니다.</p></div><label className="resume-intro"><span>경력 요약·자기소개 *</span><textarea required rows="8" value={form.introduction} onChange={(e) => update('introduction', e.target.value)} placeholder="환자 진료 경험, 강점, 이직 시 중요하게 보는 조건을 적어주세요." /><small>{form.introduction.length}자</small></label><div className={`resume-file-box ${dragTarget === 'resume' ? 'is-dragging' : ''}`} {...dropZoneProps('resume', (files) => chooseResumeFile(files?.[0]))}><div><Upload /><span><strong>기존 이력서 첨부 <i>선택</i></strong><small>클릭하거나 끌어놓기 · PDF·DOC·DOCX·HWP·HWPX · 최대 10MB</small></span></div><label><input type="file" accept=".pdf,.doc,.docx,.hwp,.hwpx" onChange={(e) => chooseResumeFile(e.target.files?.[0])} />{resumeFile?.name || '파일 선택'}</label>{resumeFileError && <em>{resumeFileError}</em>}</div><fieldset className="resume-visibility"><legend>구직 공개 설정 — 인재정보에 올릴지 선택하세요</legend>{[['public','구직 등록 (인재정보에 공개)','병원 회원이 익명 프로필로 나를 검색할 수 있습니다. 이름·연락처는 열람권 결제 후에만 공개됩니다.'],['proposal','제안 올 때만 공개','평소엔 비공개이며, 맞는 병원 제안이 있을 때 동의를 요청합니다.'],['private','비공개 보관','인재정보에 올리지 않고, 내가 직접 지원할 때만 전달합니다.']].map(([value,title,copy]) => <label key={value} className={form.visibility === value ? 'active' : ''}><input type="radio" name="visibility" value={value} checked={form.visibility === value} onChange={(e) => update('visibility', e.target.value)} /><span><strong>{title}</strong><small>{copy}</small></span>{form.visibility === value && <Check />}</label>)}</fieldset><label className="resume-consent"><input type="checkbox" checked={form.consent} onChange={(e) => update('consent', e.target.checked)} /><span>이력서 등록과 채용 매칭을 위한 개인정보 수집·이용에 동의합니다. 면허·자격 정보는 검증과 채용 연결 목적으로만 사용됩니다.</span></label></div>}
