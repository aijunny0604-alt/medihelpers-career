...
import {
  ...
  CircleCheck, FileText, GraduationCap, LockKeyhole, Plus, ShieldCheck,
  Stethoscope, Trash2, Upload, UserRound
} from 'lucide-react';
  ...
  return <main className="resume-page">
    ...
    <form className="resume-layout" onSubmit={submit}>
      ...
      <section className="resume-editor">
        {activeStep === 'basic' && <div className="resume-step-panel"><div className="resume-panel-head"><small>STEP 01</small><h2>기본정보와 직군</h2><p>채용기관이 가장 먼저 확인하는 정보입니다.</p></div><div className="resume-form-grid"><label className="wide"><span>이력서 제목 *</span><input required value={form.title} onChange={(e) => update('title', e.target.value)} placeholder="예: 소화기내과 전문의 · 부산경남 이직 희망" /></label><label><span>의료 직군 *</span><select value={form.profession} onChange={(e) => update('profession', e.target.value)}>{professions.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>현재 거주지역</span><input value={form.region} onChange={(e) => update('region', e.target.value)} placeholder="예: 부산광역시" /></label><label><span>이름 *</span><input required value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="홍길동" /></label><label><span>휴대폰 *</span><input required type="tel" value={form.phone} onChange={(e) => update('phone', e.target.value)} placeholder="010-0000-0000" /></label><label className="wide"><span>이메일 *</span><input required type="email" value={form.email} onChange={(e) => update('email', e.target.value)} placeholder="doctor@example.com" /></label></div><div className="resume-safe-note"><ShieldCheck /> 주민등록번호·성별·사진은 필수로 받지 않습니다.</div></div>}
      ...
      <aside className="resume-preview"><small>LIVE PREVIEW</small><h3>{form.title || '이력서 제목을 입력해주세요'}</h3><span className="resume-preview-role">{form.profession} · {form.specialty || '전문분야 미입력'}</span><dl><div><dt>총 경력</dt><dd>{form.experienceYears || '미입력'}</dd></div><div><dt>희망 지역</dt><dd>{form.desiredRegions || '미입력'}</dd></div><div><dt>근무 형태</dt><dd>{form.workTypes.join(' · ') || '미입력'}</dd></div><div><dt>입사 가능</dt><dd>{form.available || '협의'}</dd></div></dl><div className="resume-preview-privacy"><LockKeyhole /><span><strong>연락처·면허번호 비공개</strong><small>공개 동의 전에는 채용기관에 표시하지 않습니다.</small></span></div></aside>
    </form>
  </main>;
}
