...
const recruitGeneralBannerBase64 = await readBase64('banners', 'recruit-general-v1.jpg');
const recruitSpineBannerBase64 = await readBase64('banners', 'recruit-spine-v1.jpg');
const recruitCheckupBannerBase64 = await readBase64('banners', 'recruit-checkup-v1.jpg');
const recruitWomenBannerBase64 = await readBase64('banners', 'recruit-women-v1.jpg');
...
const recruitGeneralBannerBase64 = ${JSON.stringify(recruitGeneralBannerBase64)};
const recruitSpineBannerBase64 = ${JSON.stringify(recruitSpineBannerBase64)};
const recruitCheckupBannerBase64 = ${JSON.stringify(recruitCheckupBannerBase64)};
const recruitWomenBannerBase64 = ${JSON.stringify(recruitWomenBannerBase64)};
...
const testAccountDefinitions = Object.freeze({
  doctor: { email:'doctor-test@medihelpers.co.kr', role:'doctor', displayName:'의료인 회원', phone:'010-0000-0000' },
...
async function expireJobPosts(env) {
  // Backfill legacy deadlines separately from schema migrations; month ends clamp.
  await env.DB.prepare("UPDATE job_seeker_posts SET public_until=datetime(min(date(updated_at,'+9 hours','+5 months'),date(updated_at,'+9 hours','start of month','+6 months','-1 day')) || ' ' || time(updated_at,'+9 hours'),'-9 hours') WHERE public_until='' AND status<>'deleted'").run();
  await env.DB.batch([
    env.DB.prepare("INSERT INTO member_activity (id,account_id,event_type,title,detail) SELECT 'auto-hide-' || id || '-' || public_until,account_id,'job_seeker_post_auto_hide','5개월 동안 관리하지 않은 구직글이 자동 비공개되었습니다.',title FROM job_seeker_posts WHERE status='active' AND public_until<=CURRENT_TIMESTAMP ON CONFLICT(id) DO NOTHING"),
    env.DB.prepare("UPDATE job_seeker_posts SET status='closed', hidden_reason='inactive' WHERE status='active' AND public_until<=CURRENT_TIMESTAMP")
  ]);
}
async function ensureMemberCenterSchema(env) {
  ...
  if (!schemaReadyPromises.has('multiple-job-posts-v1')) schemaReadyPromises.set('multiple-job-posts-v1',env.DB.prepare('DROP INDEX IF EXISTS job_seeker_posts_active_resume_idx').run().catch(error=>{schemaReadyPromises.delete('multiple-job-posts-v1');throw error;}));
  await schemaReadyPromises.get('multiple-job-posts-v1');
  await expireJobPosts(env);
...
const backupTables = [
  ...
  'talent_credit_pools','data_protection_runs','hospital_verification_requests'
];
...
async function authApi(request, env, pathname, ctx) {
  ...
  const multipartRegistration = pathname === '/api/auth/register' && contentType.includes('multipart/form-data');
  const length = Number(request.headers.get('content-length') || 0);
  if (length > (multipartRegistration ? 12 * 1024 * 1024 : 32768)) return json({ error:'요청 크기가 너무 큽니다.' }, 413);
  let body;
  let businessDocument = null;
  try {
    if (multipartRegistration) {
  ...
  if (pathname === '/api/auth/change-password') {
    const identity = await authenticatedUser(request, env);
    if (!identity?.accountId) return json({error:'로그인 후 비밀번호를 변경해주세요.'},401);
    if (!validPassword(body.password)) return json({error:'새 비밀번호는 영문과 숫자를 포함한 8~128자로 입력해주세요.'},400);
    const credential = await env.DB.prepare('SELECT password_hash AS hash,password_salt AS salt,password_iterations AS iterations,failed_attempts AS failures,locked_until AS lockedUntil FROM auth_credentials WHERE account_id=?').bind(identity.accountId).first();
    if (!credential || credential.lockedUntil && new Date(credential.lockedUntil + 'Z').getTime() > Date.now()) return json({error:'잠시 후 다시 시도해주세요.'},429);
    const currentHash = await passwordHash(String(body.currentPassword || ''),credential.salt,credential.iterations);
    if (!constantTimeEqual(currentHash,credential.hash)) {
      await env.DB.prepare("UPDATE auth_credentials SET failed_attempts=failed_attempts+1,locked_until=CASE WHEN failed_attempts>=4 THEN datetime('now','+15 minutes') ELSE locked_until END WHERE account_id=?").bind(identity.accountId).run();
      return json({error:'현재 비밀번호가 일치하지 않습니다.'},400);
    }
    if (body.password === body.currentPassword) return json({error:'현재 비밀번호와 다른 새 비밀번호를 입력해주세요.'},400);
    const salt = randomHex(16);
    const hash = await passwordHash(body.password,salt);
    const results = await env.DB.batch([
      env.DB.prepare('UPDATE auth_credentials SET password_hash=?,password_salt=?,password_iterations=?,failed_attempts=0,locked_until=NULL,updated_at=CURRENT_TIMESTAMP WHERE account_id=? AND password_hash=?').bind(hash,salt,passwordIterations,identity.accountId,credential.hash),
      env.DB.prepare('DELETE FROM auth_sessions WHERE account_id=? AND changes()=1').bind(identity.accountId),
      env.DB.prepare('UPDATE account_password_resets SET used_at=CURRENT_TIMESTAMP WHERE account_id=? AND used_at IS NULL').bind(identity.accountId)
    ]);
    if (!runChanges(results[0])) return json({error:'계정 상태가 변경되었습니다. 다시 로그인해주세요.'},409);
    return json({changed:true},200,{'set-cookie':authCookie('',0)});
  }
  ...
  if (pathname === '/api/auth/register') {
    ...
    if (body.role === 'hospital') {
      ...
      if (!hospitalName || !representativeName || businessNumber.length !== 10 || !address) return json({ error:'병원명·대표자명·사업자등록번호·주소를 확인해주세요.' }, 400);
      if (!businessDocument) return json({ error:'사업자등록증 PDF 또는 이미지 파일을 첨부해주세요.' }, 400);
    ...
    if (body.role === 'hospital') {
      ...
      const bytes = await businessDocument.arrayBuffer();
      const view = new Uint8Array(bytes);
      const type = String(businessDocument.type || '').toLowerCase();
      const signatureOk = type === 'application/pdf' ? String.fromCharCode(...view.slice(0,5)) === '%PDF-' : type === 'image/jpeg' ? view[0] === 0xff && view[1] === 0xd8 : type === 'image/png' ? view[0] === 0x89 && view[1] === 0x50 && view[2] === 0x4e && view[3] === 0x47 : type === 'image/webp' ? String.fromCharCode(...view.slice(0,4)) === 'RIFF' && String.fromCharCode(...view.slice(8,12)) === 'WEBP' : false;
      if (!signatureOk) {
        if (createdAccount) try { await env.DB.prepare('DELETE FROM accounts WHERE id=?').bind(account.id).run(); } catch {}
        return json({ error:'파일 내용과 확장자가 일치하지 않습니다. 원본 사업자등록증 파일을 다시 선택해주세요.' }, 400);
      }
      const ext = { 'application/pdf':'pdf', 'image/jpeg':'jpg', 'image/png':'png', 'image/webp':'webp' }[type];
      storedDocumentKey = 'verifications/hospitals/' + account.id + '/business-registration/' + crypto.randomUUID() + '.' + ext;
      const digest = await crypto.subtle.digest('SHA-256', bytes);
      verificationRecord = { id:crypto.randomUUID(), hospitalName:organization, representativeName:String(body.representativeName || '').trim().slice(0,80), businessNumber:String(body.businessNumber || '').replace(/[^0-9]/g, '').slice(0,10), address:[body.address, body.addressDetail].filter(Boolean).join(' ').trim().slice(0,300), documentKey:storedDocumentKey, originalFilename:String(businessDocument.name || 'business-registration.' + ext).replace(/[^a-zA-Z0-9가-힣._ -]/g,'_').slice(0,180), contentType:type, fileSize:Number(businessDocument.size || bytes.byteLength), fileSha256:bytesToHex(new Uint8Array(digest)) };
      try { await uploadStorage.put(storedDocumentKey, bytes, { httpMetadata:{ contentType:type, contentDisposition:'inline' }, customMetadata:{ accountId:account.id, purpose:'hospital-verification', sha256:verificationRecord.fileSha256 } }); }
      catch {
        if (createdAccount) try { await env.DB.prepare('DELETE FROM accounts WHERE id=?').bind(account.id).run(); } catch {}
...
// 병원 배너·로고·시설 사진과 의료인 이력서 프로필 사진 업로드 API.
...
async function uploadApi(request, env, pathname) {
  ...
  if (request.method === 'GET') {
    ...
    // [보안] 경로 이탈 방지. 우리가 발급한 병원·프로필 키만 서빙한다.
    if (!key || key.indexOf('..') !== -1 || (key.indexOf('hospitals/') !== 0 && key.indexOf('profiles/') !== 0)) return json({ error:'잘못된 요청입니다.' }, 400);
    // 증명사진은 배너처럼 공개 자산이 아니다. 본인·관리자 또는 해당 후보 열람권을 가진 병원만 본다.
    ...
    headers.set('cache-control', key.indexOf('profiles/') === 0 ? 'private, no-store' : 'public, max-age=31536000, immutable');
  ...
  if (request.method === 'POST') {
    ...
    if (isResumeProfile) {
      if (!account || account.role !== 'doctor') return json({ error:'이력서 사진은 의료인 회원 본인만 업로드할 수 있습니다.' }, 403);
    } else if (!isAdmin && (!account || account.role !== 'hospital')) return json({ error:'병원 회원 또는 관리자만 이미지를 업로드할 수 있습니다.' }, 403);
...
async function memberCenterApi(request, env) {
  ...
  if (request.method === 'POST') {
    ...
    if (body.action === 'owned_ad_update') {
      ...
      const registered = await env.DB.prepare('SELECT organization FROM member_profiles WHERE account_id=?').bind(account.id).first();
      const hospital = s(registered?.organization || source.hospital, 180);
  ...
  if (request.method === 'PATCH') {
    ...
    const proposed = body.profile && typeof body.profile === 'object' ? body.profile : {};
    if (Object.keys(proposed).some(key => ['displayName','phone','organization','jobTitle'].includes(key) && String(proposed[key] || '') !== String(previousProfile?.[key] || ''))) return json({ error:'가입한 회원정보는 직접 변경할 수 없습니다. 정보 정정은 고객센터에 문의해주세요.' },403);
    const profile = cleanMemberProfile(previousProfile || {});
...
async function resumeApi(request, env) {
  ...
  if (request.method === 'POST') {
    ...
    // 연락처는 구조화 컬럼에만 저장한다. detail_json에 중복 저장하면 공개 응답에서
    // 이름·전화·이메일을 하나씩 가려도 JSON 내부에서 다시 노출될 수 있다.
    delete detail.name; delete detail.phone; delete detail.email; delete detail.consent;
    detail.contactVisibility = contactVisibility;
...
async function jobSeekerPostApi(request, env, pathname) {
  ...
  if (current && body.action === 'set_visibility') {
    if (!['active','closed'].includes(body.status)) return json({error:'공개 상태를 확인해주세요.'},400);
    if (body.status === 'active' && (body.publicationAcknowledged !== true || body.privacyVersion !== PRIVACY_FORM_VERSION)) return json({error:'구직글 공개 범위를 확인해주세요.'},400);
    if (body.status === 'active') {
      const linked = await env.DB.prepare('SELECT profession,specialty,desired_regions AS desiredRegions,detail_json AS detailJson FROM resumes WHERE id=? AND account_id=?').bind(current.resumeId,account.id).first();
      const missing = resumePublicationMissing({...linked,detail:parseJsonObject(linked?.detailJson) || {}});
      if (missing.length) return json({error:'공개할 이력서를 보완해주세요: '+missing.join(', '),missingFields:missing},400);
    }
    const publicUntil = postPublicUntil();
    await env.DB.batch([
      ...(body.status === 'active' ? [consentEvent(env,account.id,'posting',current.id)] : []),
      env.DB.prepare("UPDATE job_seeker_posts SET status=?,hidden_reason=?,public_until=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND account_id=? AND status<>'deleted'").bind(body.status,body.status === 'closed' ? 'manual' : '',publicUntil,current.id,account.id),
      env.DB.prepare("INSERT INTO member_activity(id,account_id,event_type,title,detail) VALUES (?,?, 'job_seeker_post_visibility',?,?)").bind(crypto.randomUUID(),account.id,body.status === 'active' ? '구직글을 다시 공개했습니다.' : '구직글을 비공개로 변경했습니다.',current.title)
    ]);
    return json({saved:true,post:{id:current.id,status:body.status,hiddenReason:body.status === 'closed' ? 'manual' : '',publicUntil}});
  }
...
async function talentDetailApi(request, env, pathname) {
  ...
  if (!seekerPostId && resumeId && !isOwner && !isAdmin && await env.DB.prepare("SELECT id FROM job_seeker_posts WHERE resume_id=? AND status<>'active' LIMIT 1").bind(resumeId).first()) return json({unlocked:false,detail:null,error:'비공개 구직 정보입니다.'},404);
...
async function paymentOrderApi(request, env) {
  ...
  if (body.renewContentId) {
    if (product.type !== 'doctor_ad') return json({error:'공고 상품을 확인해주세요.'},400);
    renewalContent = await env.DB.prepare("SELECT c.id,c.title,c.payload_json AS payloadJson,o.metadata_json AS metadataJson FROM admin_content_records c JOIN payment_orders o ON json_extract(o.metadata_json,'$.contentRecordId')=c.id WHERE c.id=? AND o.account_id=? AND o.product_id=? AND o.status='paid' ORDER BY o.paid_at DESC LIMIT 1").bind(String(body.renewContentId),account.id,productId).first();
    if (!renewalContent) return json({error:'재노출할 본인 공고를 찾을 수 없습니다.'},404);
    const old = parseJsonObject(renewalContent.payloadJson) || {};
    if (!isAdExposureExpired(old)) return json({error:'노출 기간이 끝난 공고만 재구매할 수 있습니다.'},409);
    const pending = await env.DB.prepare("SELECT id,order_number AS orderNumber FROM payment_orders WHERE account_id=? AND status IN ('pending_review','awaiting_payment') AND json_extract(metadata_json,'$.renewalContentId')=? AND created_at>datetime('now','-30 minutes') LIMIT 1").bind(account.id,renewalContent.id).first();
    if (pending) return json({error:'진행 중인 재구매가 있습니다. 같은 주문의 결과를 확인해주세요.',recoveryOrder:{orderNumber:pending.orderNumber}},409);
    Object.assign(metadata,parseJsonObject(renewalContent.metadataJson) || {},old,{title:renewalContent.title,renewalContentId:renewalContent.id});
    delete metadata.exposure; delete metadata.exposureEnd;
  }
...
async function adminConsoleApi(request, env, ctx) {
  ...
  if (request.method === 'GET') {
    ...
    return json({
      ...
      metrics: {
        ...
        payments:Number(paymentMetrics?.total || 0), pendingPayments:Number(paymentMetrics?.pending || 0), paidRevenue:Number(paymentMetrics?.paid_revenue || 0), refundedPayments:Number(paymentMetrics?.refunded || 0), pendingHospitalVerifications:Number(verificationMetric?.pending || 0)
      },
      settings, features,
  ...
  if (action === 'settings_update') {
    ...
    if (String(env.PAYMENT_LIVE) === 'true' || env.INICIS_MID || env.INICIS_SIGN_KEY) return json({error:'실결제 환불은 PG 취소 결과 검증 연동이 필요합니다. 환불 완료로 기록할 수 없습니다.'},503);
    ...
    if (fullyRefunded) {
      ...
      if (meta.contentRecordId) statements.push(env.DB.prepare("UPDATE admin_content_records SET status='hidden',updated_at=CURRENT_TIMESTAMP WHERE id=? AND NOT EXISTS (SELECT 1 FROM payment_orders WHERE id<>? AND status='paid' AND json_extract(metadata_json,'$.contentRecordId')=? AND json_extract(metadata_json,'$.exposure.end')>=date('now'))").bind(meta.contentRecordId,orderId,meta.contentRecordId));
    } else {
    ...
    if (String(env.PAYMENT_LIVE) === 'true' || env.INICIS_MID || env.INICIS_SIGN_KEY) return json({error:'실결제 환불은 PG 취소 결과 검증 연동이 필요합니다.'},503);
...
async function refundReviewApi(request,env) {
  ...
  try {
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO payment_transactions (id,order_id,transaction_type,provider,provider_transaction_id,amount,status,processed_at) VALUES (?,?,'refund','virtual',?,?,'succeeded',CURRENT_TIMESTAMP)").bind(crypto.randomUUID(),refund.orderId,refund.id,refund.totalAmount),
      ...
      env.DB.prepare("INSERT INTO payment_events (id,order_id,actor_key,event_type,detail_json) VALUES (?,?,?,'virtual_refund_completed',?)").bind(crypto.randomUUID(),refund.orderId,(await authenticatedUser(request,env)).email,JSON.stringify({refundId:refund.id,amount:refund.totalAmount}))
    ]);
...
async function responseFor(request, env, ctx) {
  ...
  if (pathname === '/api/auth/change-password' || pathname === '/api/auth/register' || pathname === '/api/auth/login' || pathname === '/api/auth/logout' || pathname === '/api/auth/test-switch') return authApi(request, env, pathname, ctx);
...
${inlineAssets ? `  if (pathname === '/og-medihelpers.jpg') return new Response(binary(ogBase64), { status: 200, headers: { 'content-type': 'image/jpeg', 'cache-control': 'public, max-age=86400' } });
  ...
  if (pathname === '/banners/recruit-general-v1.jpg') return new Response(binary(recruitGeneralBannerBase64), { status: 200, headers: { 'content-type': 'image/jpeg', 'cache-control': 'public, max-age=31536000, immutable' } });
  if (pathname === '/banners/recruit-spine-v1.jpg') return new Response(binary(recruitSpineBannerBase64), { status: 200, headers: { 'content-type': 'image/jpeg', 'cache-control': 'public, max-age=31536000, immutable' } });
  if (pathname === '/banners/recruit-checkup-v1.jpg') return new Response(binary(recruitCheckupBannerBase64), { status: 200, headers: { 'content-type': 'image/jpeg', 'cache-control': 'public, max-age=31536000, immutable' } });
  if (pathname === '/banners/recruit-women-v1.jpg') return new Response(binary(recruitWomenBannerBase64), { status: 200, headers: { 'content-type': 'image/jpeg', 'cache-control': 'public, max-age=31536000, immutable' } });
...
if (!inlineAssets) {
  ...
  const wrangler = [
    ...
    'directory = "./public"',
    'binding = "ASSETS"',
    '# HTML의 canonical·no-store와 API 권한 검사는 반드시 Worker를 통과한다.',
    ...
    '# 운영 준비 기본값: 테스트 로그인 차단, 키 누락 시 가상 승인 금지.',
