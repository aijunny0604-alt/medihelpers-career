...
const resume=await call('/api/resumes','doctor',{...consent,title:'연동 검수 이력서',name:'가상 의료인',phone:'01000000000',profession:'의사',specialty:'내과',createNew:true,detail:{photoUrl:photo.data.url,introduction:'합성 시험 자료'}});
...
record('duplicate publication rejected',(await call('/api/job-seeker-posts','doctor',postInput)).status,409);
...
const application={...consent,thirdPartyConsent:true,recipient:'연동 검수 병원',requestType:'doctor',payload:{name:'가상 의료인',phone:'01000000000',specialty:'내과',jobId:'admin-'+ad.contentRecordId,resumeId:resume.data.id}};
...
await call('/api/resumes','doctor',{...consent,resumeId:resume.data.id,title:'지원 이후 수정',name:'가상 의료인',phone:'01000000000',detail:{introduction:'변경'}});
...
record('contact opt-in reflected',(await call(talent,'hospital')).data.detail?.phone,'01000000000');
